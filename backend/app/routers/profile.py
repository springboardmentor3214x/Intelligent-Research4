...
def _build_profile_response(profile: ResearchProfile, user: User) -> ResearchProfileResponse:
    """Helper to assemble combined ResearchProfile + User response model."""
...
def create_profile(
    ...
    # Update User fields if provided in creation payload
...
def get_profile(
    ...
    - Returns 404 Not Found if the research profile does not exist.
    """
    profile = (
        db.query(ResearchProfile)
        .filter(ResearchProfile.user_id == current_user.id)
...
def update_profile(
    ...
    - Returns 404 Not Found if the research profile does not exist.
    - Updates ResearchProfile fields: research_domain, research_interests.
    - Updates User fields: name, organization, designation, country, phone_number.
